// emcc -o hello2.html engcalc.cpp -O3 -s WASM=1  -s
// "EXTRA_EXPORTED_RUNTIME_METHODS=['ccall','cwrap', 'UTF8ToString',
// 'getValue']"  -s ASSERTIONS=1 -s ALLOW_MEMORY_GROWTH=1
...
long double constants[5] = {M_PI, M_E, (1.0 + sqrt(5)) / 2, 0.57721566490153,
                            0.007297352566417};
vector<pdv> pairs;

static vector<string> cst = {"\\pi ", "e", "\\varphi ", "\\gamma ", "\\alpha "};
// static vector<string> cst = {"pi ","e ","GoldenRatio ","EulerGamma
// ","300000000 "};
...
string EMSCRIPTEN_KEEPALIVE latexfy(vector<int> v) {
  ...
  for (int i = 0; i < v.size(); i++) {
    ...
    if (v[i] >= 5 && v[i] < 10) {
      ...
      if (v[i] == 8) ep = "\\frac {" + ex[ind - 2] + "}{" + ex[ind - 1] + "}";
      if (v[i] == 9) {
        if (exodr[ind - 2] < 3)
  ...
  for (int i = 0; i < v.size(); i++) {
    if (v[i] < 5) {
      ...
      if (v[i] == 5)
        ...
        if (abs(b) < 1e-6) return DBL_MAX;
        ans = a / b;
      } else if (v[i] == 9) {
        if (a < 1e-6) return DBL_MAX;
        ans = pow(a, b);
      }
...
pdv EMSCRIPTEN_KEEPALIVE approximate(long double target, long double eps) {
  ...
  int idx =
      lower_bound(pairs.begin(), pairs.end(), pdv(target, vector<int>())) -
      pairs.begin();
...
const char* EMSCRIPTEN_KEEPALIVE run(char* targetString) {
  ...
  cout << fixed << endl;
  stream << latexfy(p.second) << " = " << fixed << setprecision(precision)
         << p.first << endl;
